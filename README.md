...
- 백엔드 회귀 테스트 37건을 추가했다(소유권 검사 7, claim 정규화/정착 24, 사용량·쿼터 6 — parametrize 전개 기준).
...
- 자동화된 테스트는 37건 있지만 전부 유닛 수준이다(소유권 검사 로직, claim 정규화/정착 헬퍼, 사용량·쿼터 계산). 10단계 파이프라인 전체를 실행하는 통합 테스트나, LLM 응답을 목킹한 회귀 스위트는 없다 — 이번 두 차례 실행 모두 실제 API를 직접 호출해 수동으로 확인한 것이지 자동화된 스위트로 남지 않았다.
