...
이 중 `evidence_validator`의 LLM 재검증(`validate()`)은 이제 에디터 UI에서 직접 트리거하지 않는다 — 이유와 대체 기능은 아래 「AI 첨삭」 섹션 참고. `defense_engine`은 여전히 이 파이프라인 위에서 동작하지만, 지금은 생성 시점에 자동으로 매겨진 claim 상태(추정치)만으로 FLAGGED 후보를 고른다.

### 04. AI 초안이 잘 쓰였는지는 아무도 안 알려준다

사실관계 검증(위 03)과 글쓰기 품질은 다른 축이다. 사용자가 원한 건 "이 주장이 사실이냐"가 아니라 — 그건 본인이 제일 잘 안다 — "이 문장은 왜 약한지, 어떻게 고치면 나은지, 어디는 잘 썼는지"였다. `document_engine`에 `critique_document`를 추가해 생성된 문서 전체를 문장/구간 단위로 훑어 `강점`/`보완 필요`/`표현 추천` 세 카테고리로 태그하고, 각 태그에 구체적인 코멘트를 붙인다. 문서 전체의 구성·흐름에 대한 총평도 함께 준다.
...
- **"한국어로 써라"는 프롬프트가 지켜지지 않았다.** 6개 모듈이 프롬프트로 한국어 출력을 요구했지만 코드로 확인하는 곳은 한 곳도 없었다. 실제 리서치 실행 결과에서 일본어 접속사가 한국어 조사에 붙어 나왔다 — `その際에 어떤 기준으로`, `それが 작품의`. 프롬프트를 더 강하게 쓰는 건 더 큰 소리로 부탁하는 것일 뿐이라, `korean_guard`로 코드에서 강제하도록 바꿨다. 카나·한자를 결정적으로 검출하고(라틴 문자는 허용 — `TechNova`, `GA4` 같은 고유명사는 정상), 걸린 문자열만 작은 LLM 호출로 다시 쓰게 한다. 8k 토큰짜리 종합 생성을 접속사 하나 때문에 통째로 재실행하지 않기 위해서다. 재작성마저 실패하면 원본을 유지하고 로그만 남긴다 — 눈에 띄는 접속사 하나가 결과 전체를 잃는 것보다 낫다.
...
- 프론트엔드 UI/UX를 전면 개편했다: 디자인 토큰 통일, 상시 사이드바 도입, 에디터 본문에 검증 결과를 직접 밑줄로 표시(클릭 시 우측 카드와 연동), 기업·직무 리서치·면접 방어 전체화면·결과 리포트 신설(리포트는 이후 면접 방어의 탭으로 합쳤다 — 「정보 구조 재편」 참고).
...
- 백엔드 회귀 테스트 69건을 추가했다(소유권 검사 7, claim 정규화/정착 24, 사용량·쿼터 6, 한국어 출력 강제 32 — parametrize 전개 기준).
- 헤더에 문의 채널을 붙였다(`contact_inquiries`). 데모 단계라 메일 발송 인프라 없이 DB에 적재만 하고, 확인은 수동이다.

## 결제와 사용량 제한

마지막 커밋에서 들어왔고, 지금까지 이 README에 빠져 있던 부분이다.

- **무료 월 10회** — 달력 기준 한 달에 AI 동작 10회까지는 크레딧 잔액과 무관하게 무료다 (`app/core/plans.py` `FREE_MONTHLY_QUOTA`). 초과 후 크레딧이 없으면 429로 막는다.
- **크레딧 일회성 충전** — 4,900원 20회 / 9,900원 50회 / 19,900원 120회 (`CREDIT_PACKS`). Toss Payments 연동.
- **왜 구독이 아니라 충전인가** — 원래 Stripe 월 구독 티어로 만들었다가 같은 날 Toss 일회성 충전으로 갈아엎었다. 정기결제(빌링키)는 별도 심사가 필요하고 한국 시장 제품에 충전 방식이 더 맞다고 판단했다. 마이그레이션 주석에 적어뒀듯 **Stripe로 실제 결제가 흐른 적은 없다**.
- **실패한 생성에는 과금하지 않는다** — `record_usage`는 LLM 호출이 성공한 뒤에만 부른다. 실패한 생성이 사용자의 한정된 할당량을 깎으면 안 되기 때문이다.
- **단, 스트리밍은 선과금이다** — 이미 토큰을 흘려보내기 시작한 요청은 중간에 실패해도 1회가 차감된다. 사용자 입장에선 손해이고, trade-off로 라우터 주석에 남겨뒀다.
- **에디터 자동저장은 무료다** — LLM을 부르지 않으므로 쿼터를 쓰지 않는다. `reextract_claims=true`로 claim을 다시 뽑을 때만 차감한다.

**검증 상태**: 쿼터 계산 로직에 유닛 테스트 6건이 있다. 반면 **실제 Toss 결제를 태워본 적은 없다.** `TOSS_SECRET_KEY`/`TOSS_CLIENT_KEY` 기본값이 빈 문자열이고, 키가 없으면 결제 생성이 503으로 막힌다.

`ADMIN_EMAILS`(쉼표 구분)에 들어간 계정은 이 쿼터를 건너뛴다. 소유자가 자기 서비스를 테스트하다 무료 10회를 소진하는 문제 때문에 넣었고, 실제로 이 세션에서 업로드 기능을 반복 테스트하다 한도에 막혀 추가했다.

## AI 첨삭 — 근거 검증 탭을 교체했다

에디터의 두 번째 탭이 원래 "근거 검증"(evidence_validator, 위 03)이었다. 실제 써보니 문제였다 — 자기가 등록한 자기 경험을 AI가 "사실이 맞는지" 재확인해주는 건 정작 사용자가 원하는 피드백이 아니었다. 원했던 건 글쓰기 자체에 대한 첨삭: 이 부분은 왜 약한지, 어떤 전개가 나은지, 이 표현이 왜 좋은지.

- 탭을 통째로 교체했다. `document_engine.critique_document()`가 문서 전체(+ JD 요구사항, 전략 gap)를 컨텍스트로 받아 문장/구간을 `strength`/`improvement`/`suggestion`으로 태그하고 코멘트를 붙이며, 문서 전체 구성에 대한 총평(`overall_comment`)도 함께 반환한다.
- claim 하이라이트에 쓰던 형광펜 UX(정확 문자열 매칭으로 본문에 밑줄 → 클릭 시 사이드 카드로 스크롤)를 그대로 재사용했다. `claims.ts`/`DocumentCanvas.tsx`는 첨삭 전용 `critique.ts`/`CritiqueCanvas.tsx`로 대체하고 삭제했다 — 교체 후 그 두 파일을 참조하는 곳이 하나도 안 남았기 때문이다.
- `evidence_validator` 백엔드 자체는 남겨뒀다. `defense_engine`이 여전히 `claims.status`/`defense_score`를 읽어 방어질문을 만들기 때문에, 뜯어내면 면접 방어 기능이 깨진다. 다만 이걸 호출하는 UI가 이제 없으므로, 리포트 화면에서 항상 비어 있던 "근거 검증" 카드(`DefenseReport.tsx`)는 제거했다 — 절대 채워지지 않을 빈 칸을 남겨두는 게 더 나쁘다고 판단했다.
- 첨삭 결과는 DB에 저장하지 않는다. claim과 달리 이 결과를 나중에 다시 읽어야 할 이유가 없어서(첨삭은 매번 그 시점 본문에 대한 의견이지, 추적해야 할 상태가 아니다), 탭을 열 때마다 새로 요청한다.

### 왜 이 한 기능만 GPT로 옮겼나

첨삭은 체감상 NVIDIA 카탈로그(nemotron)보다 GPT 계열 결과가 더 낫다는 판단이 있었다. 그렇다고 앱 전체를 갈아엎을 이유는 없어서 — 비용도 걸리고, 나머지 9개 모듈은 이미 NVIDIA로 실측 검증까지 끝낸 상태다 — `critique_document` 이 한 호출만 별도 provider로 라우팅했다.

- `LLMGateway`에 `evaluate_json_openai()`를 `evaluate_json()`과 별도 메서드로 추가했다. 몸통을 공유하지 않은 이유: GPT-5 계열은 `temperature`에 기본값(1) 외의 값을 주면 400을 낸다(그래서 아예 파라미터를 생략한다) — 이 프로젝트의 다른 모든 LLM 호출이 쓰는 `temperature=0.1`(재현성 확보용)과 정면으로 다르다 — 그리고 `max_tokens` 대신 `max_completion_tokens`를 쓴다. 두 provider가 받는 파라미터가 겹치지 않는 지점이라 분기보다 분리가 나았다.
- 모델은 `gpt-5-mini`로 정했다. 가장 싼 `gpt-5-nano`(입력 $0.05/M, 출력 $0.40/M)도 있지만 한국어 표현 판단처럼 미묘한 작업에는 격이 낮을 수 있다고 보고, `gpt-5-mini`(입력 $0.25/M, 출력 $2.00/M)를 기본값으로 뒀다. 호출당 입력 3천~출력 1천5백 토큰 수준이면 회당 $0.003 안팎이라 비용 자체는 사실상 무시할 수준이고, `.env`의 `OPENAI_CRITIQUE_MODEL` 하나만 바꾸면 코드 수정 없이 nano로 내려갈 수 있다.
- 한국어 강제(`korean_guard.enforce_korean`)는 GPT 응답에도 그대로 걸리지만, 교정이 필요할 때 실제로 도는 보정 호출은 여전히 NVIDIA Critic 모델을 쓴다(`enforce_korean`이 provider를 가리지 않고 `llm.evaluate_json`을 부르기 때문) — 드물게 도는 보정 한 번 때문에 OpenAI 비용이 추가로 붙지 않는다.

**검증 상태**: `tsc --noEmit` 통과, 백엔드 `pytest` 87건 통과, FastAPI 앱이 새 라우트(`POST /api/v1/documents/generate/documents/{doc_id}/critique`)를 올바른 경로로 등록하는 것과 `OPENAI_*` 설정값이 정상 로드되는 것까지 코드 레벨로 확인했다. **다만 실제 OpenAI API 키로 이 엔드포인트를 끝까지 호출해본 적은 없다** — 응답 JSON 파싱, span의 본문 내 위치 매칭, 실제 첨삭 코멘트 품질은 아직 실행으로 검증하지 않았다.

## 외부 웹 리서치 (JD 인사이트 → 기업·직무 리서치)

이전 README의 한계 항목에 "웹 검색 API 키를 아직 구성하지 않았다"고 적혀 있던 부분이다. Exa Search API를 붙여 실제로 외부 자료를 참조하도록 바꿨다.

- 기업/직무 조합으로 3개의 검색 쿼리를 던져 결과를 합치고 중복 제거해 최대 12개 소스를 모은다(`search_gateway.py`, `interview_research/service.py`).
- 모은 스니펫을 Critic 모델로 종합해 웹 인사이트·예상 면접 질문·키워드·출처를 만든다. 결과는 `interview_research` 테이블에 job당 1행으로 캐시된다.
- **페이지 진입만으로는 절대 실행하지 않는다.** 검색과 LLM 호출이 모두 비용이라 명시적으로 버튼을 눌러야만 돈다.
- 프롬프트에 근거 고정(`_GROUNDING`) 규칙을 넣었다 — 검색 결과에 없는 내용은 지어내지 말고, 출처를 반드시 남기게 했다.

**실행 중 발견한 문제**: 실제 Exa 결과(스니펫 합계 21,000자 이상)로 처음 돌렸을 때 `max_tokens=4096`으로는 응답이 잘려 `JSONDecodeError`가 났다. 8192로 올려 재실행하니 12개 소스에서 웹 인사이트 5건 + 예상 질문 6건이 정상적으로 나왔다.

## 기존 문서 가져오기

지금까지는 AI 초안 생성이 유일한 시작점이었다. 이미 써둔 이력서·자소서가 있는 사용자는 진입할 방법이 없었다.

- 에디터에서 **PDF / DOCX / TXT 업로드** 또는 **본문에 직접 붙여넣기** 둘 다 가능하다. 업로드하면 기존 `document_parser`가 텍스트를 추출하고 3C4P 섹션까지 파싱한다.
- 추출된 본문은 `documents/import`로 저장된다. 기존 `_save_generated_document` 경로를 재사용해서, 업로드한 문서도 AI가 생성한 문서와 똑같이 claim 추출 → 근거 검증 → 면접 방어로 이어진다.
- 업로드는 claim 추출 LLM 호출이 따라붙으므로 다른 AI 동작과 동일하게 쿼터를 차감한다.

**실행 중 발견한 문제**: `GET /documents/{id}`가 500을 뱉었다. `documents.parsed_data`가 JSONB라 드라이버가 이미 dict로 반환하는데 코드가 `json.loads()`를 한 번 더 부르고 있었다 — README 「실패와 수정」에 적힌 `jobs.jd_analysis`와 **완전히 같은 버그**가 이 모듈에 남아 있었던 것이다. 이 엔드포인트를 프론트에서 호출한 적이 한 번도 없어서 여태 드러나지 않았다.

**브라우저 검증**: 실제 배포된 프로덕션에서 파일 업로드 → 파싱 → 에디터 본문 반영 → 새로고침 후 유지까지 클릭으로 끝까지 확인했고, 업로드 시점에 추출된 claim 2건이 우측 검증 패널에 미검증 상태로 뜨는 것까지 봤다.

## 정보 구조 재편

사이드바가 7개 항목을 순서 없이 나열하고 있었고, 그중 둘은 다른 화면과 같은 내용을 보여주고 있었다. 실제 지원 과정 순서로 다시 묶고 중복을 합쳤다.

- **내 자산** — 통합 대시보드, 경험 보관함 (지원 건과 무관하게 재사용되는 것)
- **지원 진행** — 기업·직무 리서치 → 지원서 작성 → 면접 방어 (한 건의 지원을 실제로 작업하는 순서)

두 화면을 합쳤다.

- **지원 이력 → 통합 대시보드**: 둘 다 같은 지원 목록을 같은 열기/삭제 동작과 함께 보여주고 있었다. 대시보드 쪽 카드가 진행 단계까지 보여줘 더 정보량이 많았으므로, 지원 이력에만 있던 생성일만 카드로 옮기고 화면을 없앴다.
- **피드백 리포트 → 면접 방어 탭**: 둘 다 활성 지원 건에 종속돼 있었고, 리포트는 에디터(근거 검증)와 면접 방어(답변 연습)의 결과를 요약한 것이다. 면접 방어 화면은 이미 헤더에 같은 방어 점수 게이지를 띄우고 있었다. "예상 질문 연습 / 결과 리포트" 두 탭으로 합쳤다.

`JD 인사이트`는 `기업·직무 리서치`로 이름을 바꿨다 — 이제 붙여넣은 JD만 보는 게 아니라 웹을 검색하고 예상 질문까지 만들기 때문이다.

## 한계

- 자동화된 테스트는 69건 있지만 전부 유닛 수준이다(소유권 검사 로직, claim 정규화/정착 헬퍼, 사용량·쿼터 계산, 한국어 출력 검출·복구). 10단계 파이프라인 전체를 실행하는 통합 테스트나, LLM 응답을 목킹한 회귀 스위트는 없다 — 이번 두 차례 실행 모두 실제 API를 직접 호출해 수동으로 확인한 것이지 자동화된 스위트로 남지 않았다.
...
- 웹 리서치가 모아온 소스의 **신뢰도를 판별하지 않는다.** Exa가 상위로 올려준 순서를 그대로 쓰고, 블로그 후기와 공식 채용 페이지를 구분하지 않는다. 프롬프트로 "검색 결과에 없는 건 지어내지 말라"는 제약은 걸었지만, 검색 결과 자체가 틀렸을 때 걸러낼 방법은 없다.
- 문서 업로드는 텍스트 레이어가 있는 PDF만 처리한다. 스캔본·이미지 PDF는 OCR이 없어 "텍스트를 추출하지 못했다"로 끝난다.
- 브라우저로 끝까지 밟아본 것은 **업로드 → 파싱 → 에디터 반영 → 새로고침 유지** 경로다. 편집·재작성·재검증 루프는 여전히 API 직접 호출로만 확인했고 브라우저 클릭으로 완주하지는 않았다.
- AI 첨삭(GPT 라우팅 포함)은 코드 레벨 검증(타입체크, 유닛 테스트, 라우트/설정 로드 확인)만 마쳤다. 실제 OpenAI 키로 `critique_document`를 끝까지 호출해 응답 파싱·span 매칭·코멘트 품질을 확인한 적은 없다.
- `evidence_validator`의 LLM 재검증은 이제 에디터 UI 어디에서도 트리거되지 않는다. `defense_engine`은 여전히 동작하지만 생성 시점에 자동으로 매겨진 claim 상태(추정치)에만 의존하게 됐고, 이게 방어질문 선별 정확도를 얼마나 떨어뜨리는지는 별도로 측정하지 않았다.
...
- OpenAI(`gpt-5-mini`) — AI 첨삭(`critique_document`) 이 한 호출만 전담. 그 외 전부 NVIDIA(위 「AI 첨삭」 참고)
...
# 선택 — 없으면 해당 기능만 비활성화된다
EXA_API_KEY=            # 기업·직무 웹 리서치
TOSS_CLIENT_KEY=        # 크레딧 충전 (없으면 결제 생성이 503)
TOSS_SECRET_KEY=
ADMIN_EMAILS=           # 쉼표 구분, 무료 쿼터를 건너뛸 계정
OPENAI_API_KEY=         # AI 첨삭. 없으면 이 기능만 비활성화(호출 시 인증 에러)
OPENAI_CRITIQUE_MODEL=gpt-5-mini   # 기본값. 더 아끼려면 gpt-5-nano로 교체 가능
```
