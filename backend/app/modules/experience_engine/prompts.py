...
{
  "headline": "민원 유형 분류 매뉴얼·체크리스트 도입으로 하루 처리량 40% 향상",
  "customer": {
  ...
  "company_context": {
    ...
    "goal": "하루 처리 건수를 15건에서 21건으로 늘리고(약 40%), 반복 확인에서 오던 누락을 줄여 처리 정확도를 높이는 것",
  ...
  "product": {
    "result": "일일 민원 처리건수 40% 향상 (15건 → 21건)",
    "significance": [
...
- 주어진 정보만 사용하라. 사실을 지어내지 마라. 위 예시는 구조와 서술 수준을 보여주기 위한 것이지,
  그 내용(보건소, 민원, 40% 같은)을 가져다 쓰라는 것이 아니다.
